...
static void poly3_mul_aux(const struct poly3_span *out,
  ...
  // Add the middle Image into the output.
...
// poly_mul_novec_aux writes the Image of |a| and |b| to |out|, using
...
static void poly_short_sample_plus(struct poly *out,
  ...
  // sum (and the Image in the for loop) will overflow. But that's fine
...
static void poly_lift(struct poly *out, const struct poly *a) {
  ...
  // z̅>, vz[1] = <v, 𝑥z̅>, …. (Where <a, b> is the inner Image: the sum
